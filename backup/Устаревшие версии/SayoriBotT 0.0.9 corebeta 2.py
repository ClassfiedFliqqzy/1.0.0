...
from telebot import types; #from something import somethings
...
time.sleep(1); print(Fore.LIGHTYELLOW_EX + "\nБот запущен на версии 0.0.9 ч.2 Corebeta, активирование функции логгирования чатов...", Fore.CYAN + "OK")
...
with open('!replies/listreplies.json', 'r', encoding='utf-8') as f:
    data = json.load(f) ## Подгружаем данные с нашего json файла со списками для Сайори
lis = data['lis']; lis2 = data['lis2']; lis3 = data['lis3']; lis5 = data['lis5']; lis6 = data['lis6']
...
react1 = data['react1']; react3 = data['react3']; react4 = data['react4']; keyword12 = data['keyword12']; keyword13 = data['keyword13']; keyword14 = data['keyword14']; keyword15 = data['keyword15']
keyword16 = data['keyword16']; keyword17 = data['keyword17']; keyword18 = data['keyword18']; keyword19 = data['keyword19']; keyword20 = data['keyword20']; keyword21 = data['keyword21']; keyword22 = data['keyword22']
keyword23 = data['keyword23']; keyword24 = data['keyword24']; keyword25 = data['keyword25']; keyword26 = data['keyword26']; answ15 = data['answ15']; answ16 = data['answ16']; answ17 = data['answ17']
...
def get_text_messages(message): ## Запускается handle на обработку и отправку сообщений в чаты
    ...
    if message.content_type == 'photo': content = "[🖼]"; bot.forward_message(chat_id='-1001927267798', from_chat_id=chatId,  message_id=messageId) ## Получаем фотографию
    if message.content_type == 'voice': content = "[🎙]"; bot.forward_message(chat_id='-1001927267798', from_chat_id=chatId,  message_id=messageId) ## Получаем ГС (голос)

    mainmenu = types.KeyboardButton("В главное меню 🏠") ## Создаем кнопки для главного меню и не только
    btnh = types.KeyboardButton("/help"); btne = types.KeyboardButton("/example"); btng = types.KeyboardButton("/game")
    ...
    time.sleep(0.6); bot.send_message(chat_id='-1001927267798', text=f"[ {current_time} ] [ {username} ] {content} [ Название чата: {chatName} ]")
    ...
    if content.lower() == "/start" and chatName == firstname + " (ЛС)":
        ...
        captions = reply.read(); bot.send_chat_action(chatId, "upload_photo"); bot.send_photo(chatId, welcomepictureofsayori, caption=captions, reply_markup=markup)   
    ...
    if content.lower() in ['/help', '!помощь', '/help@sunnyddlc_bot', '< первая страница']:
        if chatName == firstname + " (ЛС)": ## Если чат является личными сообщениями, то мы выведем кнопки для пользователя
            reply = open('!replies/helpservice/helplist.txt', 'r', encoding='utf8'); answer = reply.read(); bot.send_message(chatId, answer, reply_markup=helpmark, parse_mode="html"); reply.close()
        else:
        ...
        if chatName == firstname + " (ЛС)": ## Если чат является личными сообщениями, то мы выведем кнопки для пользователя
            reply = open('!replies/helpservice/helplist1.txt', 'r', encoding='utf8'); answer = reply.read(); bot.send_message(chatId, answer, reply_markup=page2mark, parse_mode="html"); reply.close()
        else:
        ...
        if chatName == firstname + " (ЛС)": ## Если чат является личными сообщениями, то мы выведем кнопки для пользователя
            reply = open('!replies/helpservice/helplist2.txt', 'r', encoding='utf8'); answer = reply.read(); bot.send_message(chatId, answer, reply_markup=page3mark, parse_mode="html"); reply.close()
        else:
    ...
    if content.lower().startswith("в главное меню") and chatName == firstname + " (ЛС)": bot.send_message(chatId, "Вы вернулись в мое главное меню, чем я могу вам помочь?", reply_markup=markup)    
    ...
    if content.lower() in ['/example', '!пример', '/example@sunnyddlc_bot', 'первая страница 1️⃣']:
        if chatName == firstname + " (ЛС)": ## Если чат является личными сообщениями, то мы выведем кнопки для пользователя
            reply = open('!replies/exampleservice/examplelist.txt', 'r', encoding='utf8'); answer = reply.read(); bot.send_message(chatId, answer, reply_markup=examplemark, parse_mode="html"); reply.close()
        else:
        ...
        if chatName == firstname + " (ЛС)": ## Если чат является личными сообщениями, то мы выведем кнопки для пользователя
            reply = open('!replies/exampleservice/examplelist1.txt', 'r', encoding='utf8'); answer = reply.read(); bot.send_message(chatId, answer, reply_markup=example2mark, parse_mode="html"); reply.close()
        else:
        ...
        if chatName == firstname + " (ЛС)": ## Если чат является личными сообщениями, то мы выведем кнопки для пользователя
            reply = open('!replies/exampleservice/examplelist2.txt', 'r', encoding='utf8'); answer = reply.read(); bot.send_message(chatId, answer, reply_markup=example3mark, parse_mode="html"); reply.close()
        else:
    ...
    if sayori1 is not None:
        ...
        elif sayori1.lower() in ['пришли фанарт', 'пришли арт', 'пришли фанарт 🎨']: ## Попросить прислать фанарт из ДДЛК
            path2 = pwd("fanart"); random_filename = str(random.choice(path2)); cg2 = open((random_filename), "rb")
            bot.send_chat_action(chatId, "upload_photo"); time.sleep(3); bot.send_photo(chatId, cg2, timeout=60) # Отправить фотографию в чат, где происходит общение
        elif sayori1.lower() in ['пришли катсцену', 'пришли cg', 'пришли катсцену 🖼']: ## Попросить прислать катсцену из ДДЛК
            path1 = pwd("sayori_cg"); random_filename = str(random.choice(path1)); cg = open((random_filename), "rb")
            bot.send_chat_action(chatId, "upload_photo"); time.sleep(3); bot.send_photo(chatId, cg, timeout=60); # Отправить фотографию в чат, где происходит общение
    ...
    if content.lower().startswith("!сайоринапиши") or content.lower().startswith("/sayoriwrite"):
        if username == "ElliotMoxiess":
            ...
            bot.send_message(chatus_id, f"{soderzhus}")
        else: bot.reply_to(message, 'К сожалению у тебя нету прав для выполнения данной команды, обратись в поддержку если ты есть есть в списке админов!') 
    ...
    if content.lower().startswith("!очистка10") or content.lower().startswith("/clean10"):
        if check_admin_rights(chatId, userId):
            ...
            for i in range(10): bot.delete_message(message.chat.id, last_message_id); last_message_id -= 1
    ...
    if content.lower().startswith("!очистка50") or content.lower().startswith("/clean50"):
        if check_admin_rights(chatId, userId):
            last_message_id = message.message_id
            for i in range(50): bot.delete_message(message.chat.id, last_message_id); last_message_id -= 1
