...
from numbersrand import sixnumber; from monthdate import russiandate; from telebot import apihelper
...
time.sleep(1); print(Fore.LIGHTYELLOW_EX + "\nБот запущен на версии 0.0.9 v.2 Pre-release, активирование функции логгирования чатов...", Fore.CYAN + "OK")
...
keyword14 = ['расскажи о кики бяд', 'что ты знаешь о кики бяд', 'кто такие кики бядовцы', 'почему кики бяд', 'расскажи о кики бяд capital',
     'какая задача у кики бядовцев', 'какая задача у кики бяд', 'что такое кики бяд', 'почему кики бядовцы', 'расскажи о акционерном обществе кики бяд']
...
def get_text_messages(message):
    ...
    if content.lower().startswith("/start"):
        welcomepictureofsayori = open(("pictures/welcome.png"), "rb")
        reply = open('!replies/welcomelist.txt', 'r', encoding='utf8')
        captions = reply.read(); bot.send_photo(chatId, welcomepictureofsayori, caption=captions)
    ...
    if content.lower().startswith("/settings") or content.lower().startswith("настройки"):
        ...
        bot.reply_to(message, "Извини, но пока что эта функция находится в разработке, но она обязательно будет реализована!")
    ...
    if content.lower().startswith("/status") or content.lower().startswith("!статус"):
        if __name__ == '__main__': bot.send_chat_action(chatId, "typing"); time.sleep(3); bot.reply_to(message, "Я нахожусь в сети, жду ваши запросы!")
    ...
    if content.lower().startswith("/help") or content.lower().startswith("!помощь"):
        reply = open('!replies/helplist.txt', 'r', encoding='utf8')
        bot.send_chat_action(chatId, "typing"); time.sleep(3); answer = reply.read(); bot.reply_to(message, answer); reply.close()
    ...
    if content.lower().startswith("!обновления") or content.lower().startswith("/update"):
        reply = open('!replies/updatelist.txt', 'r', encoding='utf8')
        bot.send_chat_action(chatId, "typing"); time.sleep(3); answer = reply.read(); bot.reply_to(message, answer); reply.close()
    ...
    if content.lower().startswith("/aboutprofile") or content.lower().startswith("о профиле"):
        reply = open('!replies/aboutprofile.txt', 'r', encoding='utf8'); about_text = reply.read()
        bot.send_chat_action(chatId, "typing"); time.sleep(3)
        bot.reply_to(message, about_text.format(firstname=firstname, lastname=lastname, username=username, chatName=chatName))

    ## Реакция Сайори на команду Вопрос, /ask (Можно задать вопрос Сайори и она ответит на него)
    if content.lower().startswith("вопрос, ") or content.lower().startswith("/inquiry, "):
    ...
    if content.lower().startswith("/wiki"):
        ...
        reply = open('!replies/wikilist.txt', 'r', encoding='utf8')
        answer = reply.read(); bot.send_message(chatId, answer, reply_markup=keyboard); reply.close()         
    ...
    if content.lower().startswith("сайори"):
        ...
        if sayori1.lower().startswith("привет"): ## Поприветствовать Сайори
            ...
            trans_text = message.text[31:]
            translator_text = open('translator.txt', 'w'); translator_text.write(trans_text); translator_text.close() 
            translator_text1  = open('translator.txt', 'r'); finaltext = translator_text1.read()
            result = translator.translate(finaltext, dest='en'); translator_text.close()
            bot.send_chat_action(chatId, "typing"); time.sleep(3)
            bot.reply_to(message, f"Перевод с русского на английский: {result.text}")
        elif sayori1.lower().startswith("переведи с английского на русский: "):
            trans_text = message.text[42:]
            translator_text = open('translator.txt', 'w'); translator_text.write(trans_text); translator_text.close() 
            translator_text1  = open('translator.txt', 'r'); finaltext = translator_text1.read()
            result = translator.translate(finaltext, dest='ru'); translator_text.close()
            bot.send_chat_action(chatId, "typing"); time.sleep(3)
            bot.reply_to(message, f"Перевод с английского на русский: {result.text}")
        ...
        elif sayori1.lower().startswith(tuple(keyword14)):
            reply = open('!replies/aboutkuku.txt', 'r', encoding='utf8')
            ...
            bot.send_chat_action(chatId, "typing"); time.sleep(3)
            random_number = message.text[24:]; splitted_text = random_number.split(); del splitted_text[1]
    ...
    if content.lower().startswith(tuple(keyword)):
        reply = open('!replies/ddlcsitelist.txt', 'r', encoding='utf8')
        bot.send_chat_action(chatId, "typing"); time.sleep(3)
        answer = reply.read(); bot.reply_to(message, answer); reply.close()
    ...
    if content.lower().startswith("!гс") or content.lower().startswith("/gs"):
        ...
        myobj.save("audio.mp3"); voice = open("audio.mp3", "rb")
        bot.send_chat_action(chatId, "record_audio"); time.sleep(3)
    ...
    if content.lower().startswith("!игра") or content.lower().startswith("/game"):
        ...
        ## Сценарий мини-игры №1 - игрок выбирает камень, Сайори выбирает.
            ...
            bot.reply_to(message, "Ты выбрал(а) камень, я выбрала: "+sayori_select); time.sleep(1)
        ...
        ## Сценарий мини-игры №2 - игрок выбирает ножницы, Сайори выбирает.
            ...
            sayori_select1 = random.choice(sayorichoose1); time.sleep(1)
            bot.reply_to(message, "Ты выбрал(а) ножницы, я выбрала: "+sayori_select1); time.sleep(1)
        ...
        ## Сценарий мини-игры №3 - игрок выбирает бумагу, Сайори выбирает.
            ...
            sayori_select2 = random.choice(sayorichoose2); time.sleep(1)
            bot.reply_to(message, "Ты выбрал(а) бумагу, я выбрала: "+sayori_select2); time.sleep(1)
            ...
            reply = open('!replies/gamelist.txt', 'r', encoding='utf8')
            answer = reply.read(); bot.reply_to(message, answer); reply.close()
        else:
            bot.reply_to(message, "Ой-ой, что-то пошло не так! Может быть ты неправильно написал название игры? Если ты не знаешь номер игры - то напиши команду /game список или же - !игра список")
...
if __name__ == '__main__': ## Проверка того, был ли запущен файл напрямую
    try:
        ...
        notification.notify( ## Создать оповещение системы
            ...
            app_icon = 'pictures/console.ico', ## Инонка для уведомления
            timeout = 5, ## Через сколько секунд пропадет уведомление
        )
